...
# > : 현재 대상의 바로 하위 child를 찾음. 여러 번 중첩하여 사용 가능함.
h1 = soup.select_one("div#cartoon > h1").string
print("h1 =", h1)

print()
# 객체.select(<선택자>) : css 선택자로 여러 요소를 '리스트'로 추출함
li = soup.select("ul.elements > li")
# li = soup.select("div#cartoon > ul.elements > li") 동일한 문장
for a in li:
    print("li =", a.string)
